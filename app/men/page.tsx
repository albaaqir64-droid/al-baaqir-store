...
export default async function Page() {
  ...
  const data = allProducts.filter(p => {
    ...
    const isMenGender = gender === "men" || gender === "unisex";

    const category = (p.category || "").trim().toLowerCase();
    const isMenCategory = menCategories.some(c => c.toLowerCase() === category);
    ...
    <div className="min-h-screen bg-[#faf8f4] text-[#151515]">
      ...
      <main className="mx-auto max-w-[1200px] px-5 py-20">
        <div className="mb-12">
          ...
          <h1 className="text-[34px] md:text-[42px] serif font-medium mt-2">Men&apos;s essentials</h1>
          <p className="mt-4 text-[#777] max-w-[600px]">Modern utility and premium Indian craftsmanship, curated for the modern man.</p>
        </div>
