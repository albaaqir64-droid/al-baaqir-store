...
export default async function Page() {
  ...
  // Case-insensitive filtering for Belts
  const data = allProducts.filter(p =>
    p.category.trim().toLowerCase() === "belts"
  );
    ...
    <div className="min-h-screen bg-[#faf8f4] text-[#151515]">
      ...
      <main className="mx-auto max-w-[1200px] px-5 py-20">
        <div className="mb-12">
          ...
          <h1 className="text-[34px] md:text-[42px] serif font-medium mt-2">Premium Belts</h1>
          <p className="mt-4 text-[#777] max-w-[600px]">Refined leather belts with mirror-polished buckles and rich finishes.</p>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="border border-[#e8e2d9] bg-white p-20 text-center">
            <p className="text-[#aaa] text-[12px] font-bold uppercase tracking-widest">No belts found</p>
          </div>
        )}
      </main>
